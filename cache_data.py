...
# NOTE v6 (revision) : une premiere version de ce fichier utilisait des
# regex de negation ("pas de secef", "sans machine" -> concept
# ABSENCE_SYSTEME) comme signal fort independant. Teste en conditions
# reelles, ce mecanisme s'est revele TROP FRAGILE : "sans secef" est un
# simple modificateur qui peut apparaitre dans des questions sur des
# sujets totalement differents ("puis-je recuperer la TVA sur une
# facture ordinaire, SANS SECeF ?" a ete faussement aiguille vers
# l'entree "absence totale de systeme" alors que la question porte sur
# la TVA). La negation est donc geree plus simplement et plus surement
# via le champ "expressions" (voir _bonus_expression ci-dessous), avec
# des phrases suffisamment specifiques (3+ mots) pour ne pas se
# declencher par coincidence - cf. les entrees "sanctions" et
# "pas-de-secef-vs-machine-inutilisee" dans cache_data.py.
...
def _score_entree(mots_question, entree, mots_question_set=None):
...
def repondre_locale(question_brute):
    ...
    for entree in QA_LIBRARY:
        score, couverture, signal_fort = _score_entree(mots_question, entree, mots_question_set)
        if score > 0:
